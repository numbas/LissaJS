...
if(!window.LissaJS) { window.LissaJS = {} }	// create the LissaJS object.

LissaJS.extensions = {};

// LissaJS.debug is a function for displaying debug info in the console. It will try to give a reference back to the line that called it, if it can.
LissaJS.debug = function(msg,noStack)
{
	if(window.console)
	{
		var e = new Error(msg);
...
// displays an error in a nice alert box. Also sends the error to the console via LissaJS.debug
LissaJS.showError = function(e)
{
	var message;
	if(e.stack)
	{
	...
	LissaJS.debug(e.stack || message);
	LissaJS.display.showAlert(message);
	throw(e);
};
...
var loadScript = LissaJS.loadScript = function(file,noreq)	
//load a javascript file. Unless noreq is set, the file's code must be wrapped in a call to LissaJS.queueScript with its filename as the first parameter
{
	if(!noreq)
	{
...
var loadCSS = LissaJS.loadCSS = function(file)
{
...
LissaJS.scriptPrefix = 'scripts/';

LissaJS.queueScript = function(file, deps, callback)	
...
{
	file = file.replace(/^scripts\//,LissaJS.scriptPrefix);
	...
	for(var i=0;i<deps.length;i++)
	{
		...
		if(!dep.match('/'))				//so can refer to built-in scripts just by name
			dep = LissaJS.scriptPrefix+deps[i]+'.js';
		deps[i] = dep;
		loadScript(dep);
...
function matchTree(ruleTree,exprTree)
{
	...
	//LissaJS.debug("matching "+treeToJME(ruleTree)+" with "+treeToJME(exprTree));
